        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 mb-0.5">
            ...
            >
              {status === "running" || status === "pending" ? (
                <>
                  <Loader2 className="animate-spin mr-0.5" size={8} />
                  RUNNING
                </>
              ) : status === "failed" ? (
                <>
                  <XCircle className="mr-0.5" size={8} />
                  FAILED
                </>
              ) : (
                <>
                  <CheckCircle className="mr-0.5" size={8} />
                  COMPLETED
                </>
              )}
            </Badge>
            ...
            <div className="p-2 space-y-1">
              {isResult(agent) && (agent.result || agent.error) && (
                <div className="rounded-lg border border-white/5 bg-[#080808] p-2 text-xs text-zinc-300 overflow-x-auto">
                  {agent.error ? (
                    <span className="text-red-400 font-mono text-[11px] whitespace-pre-wrap">
                      {agent.error}
                    </span>
                  ) : (
