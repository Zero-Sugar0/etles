...
export function SubAgentHub({ agents }: SubAgentHubProps) {
  const [selectedAgent, setSelectedAgent] = useState<SubAgentDefinition | null>(
    null
  );
    ...
    <div className="flex h-[calc(100vh-3.5rem)] sm:h-[calc(100vh-3.5rem)] md:h-screen w-full overflow-hidden bg-background">
              ...
              >
                ...
                <div className="flex items-start justify-between w-full mb-1.5 sm:mb-2 relative z-10">
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {agent.slug.includes("security") ||
                      agent.slug.includes("admin") ? (
                        <Shield className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                      ) : agent.slug.includes("engineer") ||
                        agent.slug.includes("developer") ? (
                ...
                <div className="flex flex-wrap gap-1 relative z-10">
                  {agent.department && (
                    <Badge
                      className="text-[8px] sm:text-[9px] px-1.5 sm:px-2 py-0 h-4 sm:h-4.5 bg-primary/10 text-primary rounded-md capitalize"
                      variant="secondary"
                    >
                  ...
                  {agent.departmentLeadSlug && (
                    <Badge
                      className="text-[8px] sm:text-[9px] px-1.5 sm:px-2 py-0 h-4 sm:h-4.5 bg-secondary/10 text-secondary rounded-md lowercase"
                      variant="secondary"
                    >
                  ...
                  {agent.toolkits.length > 3 && (
                    <Badge
                      className="text-[8px] sm:text-[9px] px-1.5 sm:px-2 py-0 h-4 sm:h-4.5 bg-muted/60 text-muted-foreground rounded-md"
                      variant="secondary"
                    >
                      +{agent.toolkits.length - 3}
                    </Badge>
      ...
      <div
        className={cn(
          ...
          selectedAgent ? "block" : "hidden md:block"
        )}
      >
        ...
        {selectedAgent ? (
          ...
          <div className="h-full flex flex-col items-center justify-center text-muted-foreground p-6 sm:p-8 animate-in fade-in duration-500">
            ...
            <p className="text-xs sm:text-sm max-w-[250px] sm:max-w-md text-center leading-relaxed opacity-80">
              Choose an agent from the sidebar to start a direct session, view
              its trace logs, and issue autonomous commands.
            </p>
          </div>
        )}
      </div>
