  ...
  return (
    <div className="flex-1 overflow-y-auto p-3 sm:p-4 md:p-8 space-y-4 sm:space-y-6 md:space-y-8 bg-background">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4">
        <div className="flex flex-col space-y-1 sm:space-y-2">
          ...
          <p className="text-muted-foreground text-[10px] sm:text-xs md:text-sm">
            Monitor Etles's proactive activities, automation schedules, and
            intelligence briefs.
          </p>
        </div>
            ...
            <div
              className={cn(
                ...
                status.heartbeat.status === "active"
                  ...
                  : status.heartbeat.status === "paused"
                    ? "bg-amber-500"
                    : "bg-red-500"
              )}
            />
      ...
      <Tabs className="space-y-4 sm:space-y-6" defaultValue="overview">
        ...
        <TabsContent className="space-y-4 sm:space-y-6" value="overview">
          <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <CardContent className="p-3 sm:p-6 pt-0 sm:pt-0">
                ...
                {status.heartbeat.nextRun && (
                  <div className="mt-3 sm:mt-4 flex items-center gap-1.5 sm:gap-2 text-[9px] sm:text-[10px] text-primary bg-primary/5 p-1 sm:p-1.5 px-2 sm:px-3 rounded-full w-fit">
                    ...
                    Next:{" "}
                    {new Date(status.heartbeat.nextRun).toLocaleTimeString([], {
                      hour: "2-digit",
                      minute: "2-digit",
                    })}
                  </div>
            ...
            <Card className="shadow-sm">
              ...
              <CardContent className="p-3 sm:p-6 pt-0 sm:pt-0">
                <div className="text-lg sm:text-2xl font-bold mb-2 sm:mb-3">
                ...
                <div className="flex flex-wrap gap-1 sm:gap-1.5">
                  {status.integrations.length > 0 ? (
                    status.integrations.map((i) => (
                      <Badge
                        className="capitalize text-[8px] sm:text-[10px] px-1.5 sm:px-2 py-0 h-4 sm:h-5"
                        key={i.platform}
                        variant="secondary"
                      >
                        {i.platform}
            ...
            <Card className="shadow-sm border-primary/20 bg-primary/[0.02] sm:col-span-2 lg:col-span-1">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 sm:pb-2 p-3 sm:p-6">
                <CardTitle className="text-[10px] sm:text-sm font-medium text-muted-foreground text-primary">
                  Strategic Synthesis
                </CardTitle>
          ...
          <Card className="shadow-sm overflow-hidden">
            <CardHeader className="border-b bg-muted/30 p-3 sm:p-4 md:p-6">
              ...
              <CardDescription className="text-[10px] sm:text-xs">
                A high-level synthesis of your digital signals from the last 7
                days.
              </CardDescription>
            </CardHeader>
              ...
              {status.synthesis.lastBrief ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 sm:py-20 text-center">
                  ...
                  <p className="text-[9px] sm:text-[11px] text-muted-foreground/60 max-w-[200px] sm:max-w-[250px] mt-1">
                    Your first strategic synthesis will be ready after a week of
                    activity.
                  </p>
                </div>
              )}
        ...
        <TabsContent className="space-y-4" value="automations">
          <div className="grid gap-4 sm:gap-6 grid-cols-1 lg:grid-cols-2">
            <Card className="shadow-sm">
              <CardHeader className="pb-2 sm:pb-3 border-b mb-2 sm:mb-4 p-3 sm:p-6">
                ...
                <CardDescription className="text-[10px] sm:text-xs">
                  Automations currently managed by Etles.
                </CardDescription>
              </CardHeader>
                ...
                <div className="space-y-0">
                  {status.cronJobs.some((j) => j.cron) ? (
                    status.cronJobs
                      ...
                      .map((job) => (
                        <div
                          className="group px-4 sm:px-6 py-3 sm:py-4 hover:bg-muted/50 transition-colors border-b last:border-0"
                          key={job.id}
                        >
                    ...
                    <div className="text-center py-12 sm:py-20 text-muted-foreground flex flex-col items-center">
                      ...
                      <p className="text-[10px] sm:text-xs">
                        No active recurring jobs found.
                      </p>
                    </div>
                  )}
                </div>
            ...
            <Card className="shadow-sm">
              <CardHeader className="pb-2 sm:pb-3 border-b mb-2 sm:mb-4 p-3 sm:p-6">
                ...
                <CardDescription className="text-[10px] sm:text-xs">
                  Last 5 autonomous agent activities.
                </CardDescription>
              </CardHeader>
              <CardContent className="px-0 p-0 sm:p-0">
                <div className="space-y-0">
                  {status.cronJobs.some((j) => !j.cron) ? (
                    status.cronJobs
                      ...
                      .map((job) => (
                        <div
                          className="px-4 sm:px-6 py-2.5 sm:py-3 border-b last:border-0 flex items-center justify-between gap-3 sm:gap-4"
                          key={job.id}
                        >
                          ...
                          <Badge
                            className="text-[8px] sm:text-[9px] h-3.5 sm:h-4 px-1 sm:px-1.5 capitalize text-muted-foreground shrink-0"
                            variant="outline"
                          >
                            {job.status}
                          </Badge>
