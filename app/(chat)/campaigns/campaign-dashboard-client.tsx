...
export function CampaignDashboardClient({
  ...
  const [queueItems, setQueueItems] = useState<CampaignQueueItem[]>(initialQueueItems);
  const [activeTab, setActiveTab] = useState<string>("pending_review");
  const [editingItemId, setEditingEditingItemId] = useState<string | null>(null);
  ...
  const handleToggleMission = async (mission: Mission) => {
    ...
    const toastId = toast.loading(`${nextStatus === "paused" ? "Pausing" : "Resuming"} campaign...`);
    ...
    if (res.success && res.mission) {
      ...
      toast.success(`Campaign ${nextStatus === "paused" ? "paused" : "resumed"} successfully!`, { id: toastId });
    } else {
      toast.error(res.error || "Failed to update campaign status", { id: toastId });
    }
  };

  // Action: Change Campaign Queue Item status (Approve / Reject)
  const handleUpdateItemStatus = async (id: string, nextStatus: CampaignQueueItem["status"]) => {
    ...
    if (res.success && res.item) {
      setQueueItems((prev) =>
        prev.map((item) => (item.id === id ? (res.item as CampaignQueueItem) : item))
      );
      toast.success(`Draft successfully ${nextStatus === "approved" ? "approved & queued" : "skipped"}!`, { id: toastId });
    } else {
      toast.error(res.error || "Failed to process item status", { id: toastId });
    }
  };

  // Action: Delete Campaign Queue Item
  const handleDeleteItem = async (id: string) => {
    const confirmDelete = window.confirm("Are you sure you want to permanently delete this drafted sequence?");
    ...
    if (res.success) {
      ...
      toast.error(res.error || "Failed to delete drafted item", { id: toastId });
    }
  };
  ...
  const saveEditedContent = async (id: string) => {
    ...
    if (res.success && res.item) {
      ...
      toast.error(res.error || "Failed to update draft content", { id: toastId });
    }
  };
    ...
    <div className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-8 space-y-6 sm:space-y-8 bg-background">
      ...
      <div className="grid gap-6 grid-cols-1 xl:grid-cols-3">
          ...
          <div className="space-y-4">
            {missions.length > 0 ? (
                    ...
                    <div className="p-5 space-y-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="space-y-1 min-w-0">
                          <h3 className="text-sm sm:text-base font-bold truncate pr-2" title={mission.goal}>
                            {mission.goal}
                          </h3>
                          {mission.productUrl && (
                        ...
                        >
                          <span
                            className={`mr-1.5 h-1.5 w-1.5 rounded-full inline-block ${
                              mission.status === "running" ? "bg-green-500 animate-pulse" : "bg-current"
                            }`}
                          />
                          {mission.status}
                        </Badge>
                      ...
                      <div className="space-y-2">
                        ...
                        <Progress value={percentComplete} className="h-2 rounded-full" />
              ...
              <div className="flex flex-col items-center justify-center p-8 py-16 text-center border rounded-xl border-dashed bg-sidebar/20">
                ...
                <p className="text-sm font-semibold text-muted-foreground leading-none">No Active Campaigns</p>
                <p className="text-xs text-muted-foreground/60 max-w-[200px] mt-1.5">
                  Launch a multi-week mission via chat to activate campaign automation lists.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="xl:col-span-2 space-y-6">
          <Tabs defaultValue="pending_review" className="space-y-6" onValueChange={setActiveTab}>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-sidebar-border pb-1.5">
              <h2 className="text-sm font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-2">
                <Calendar className="h-4 w-4 text-primary" /> Campaign Worklist Queue
              </h2>

              <TabsList className="grid grid-cols-4 max-w-[420px] h-9 p-1 bg-muted/60">
                <TabsTrigger value="pending_review" className="text-[10px] sm:text-xs px-2.5 rounded-md relative">
                  Review
                  {pendingCount > 0 && (
                ...
                <TabsTrigger value="approved" className="text-[10px] sm:text-xs px-2.5 rounded-md relative">
                  Approved
                  {approvedCount > 0 && (
                ...
                <TabsTrigger value="sent" className="text-[10px] sm:text-xs px-2.5 rounded-md relative">
                  Sent
                  {sentCount > 0 && (
                ...
                <TabsTrigger value="rejected" className="text-[10px] sm:text-xs px-2.5 rounded-md relative">
                  Skipped
                  {rejectedCount > 0 && (
                ...
                {filteredItems.length > 0 ? (
                        ...
                        <Card className="border overflow-hidden bg-card/60 shadow-sm hover:shadow-md transition-all duration-300">
                          ...
                          <div className="p-4 sm:p-5 border-b border-sidebar-border bg-muted/10 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                              {/* Channel Badge */}
                              <Badge className={`flex items-center gap-1.5 select-none font-bold uppercase tracking-wider ${channelConfig.style}`}>
                                {channelConfig.icon}
                                {item.channel}
                              </Badge>

                              {/* Recipient Details */}
                              <span className="text-xs font-semibold text-foreground/80 truncate max-w-[200px] sm:max-w-xs" title={item.recipient}>
                                <span className="text-muted-foreground mr-1">To:</span> {item.recipient}
                              </span>
                            </div>
                          ...
                          <div className="p-4 sm:p-5 space-y-4">
                            {isEditing ? (
                              <div className="space-y-3">
                                ...
                                <div className="flex items-center justify-end gap-2">
                                  <Button
                                    variant="ghost"
                                    size="sm"
                                    onClick={() => setEditingEditingItemId(null)}
                              ...
                              <div className="relative group/content">
                                <div className="whitespace-pre-wrap text-xs sm:text-sm leading-relaxed p-3 sm:p-4 rounded-xl bg-muted/20 border text-foreground/90 max-h-[220px] overflow-y-auto leading-relaxed">
                                  {item.content}
                                </div>
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  {item.status === "approved" && (
                                    <Button
                                      variant="outline"
                                      size="sm"
                                      onClick={() => handleUpdateItemStatus(item.id, "pending_review")}
                                  ...
                                  {item.status === "sent" && (
                                    <span className="text-xs text-green-500 font-semibold flex items-center gap-1 select-none">
                                      <CheckCircle2 className="h-4 w-4" /> Executed & Dispatched
                                    </span>
                                  )}
                                </div>
                              </div>
                  ...
                  >
                    <div className="h-12 w-12 rounded-full bg-primary/5 flex items-center justify-center mb-4">
                      {activeTab === "pending_review" && <Check className="h-6 w-6 text-green-500" />}
                      {activeTab === "approved" && <Clock className="h-6 w-6 text-primary" />}
                      {activeTab === "sent" && <ArrowRight className="h-6 w-6 text-blue-500" />}
                      {activeTab === "rejected" && <X className="h-6 w-6 text-muted-foreground" />}
                    </div>
                    <p className="text-sm font-semibold leading-none text-muted-foreground">
                      {activeTab === "pending_review" && "Review Queue Cleared"}
                      {activeTab === "approved" && "No Approved Items Scheduled"}
                      {activeTab === "sent" && "No Sent Campaigns Recorded"}
                      {activeTab === "rejected" && "No Skipped Campaign Elements"}
                    </p>
  ...
  switch (channel) {
    case "email":
      return {
        ...
        style: "bg-blue-500/10 text-blue-500 border-blue-500/20 hover:bg-blue-500/10",
      };
    case "linkedin":
      return {
        icon: <Linkedin className="h-3.5 w-3.5" />,
        style: "bg-cyan-500/10 text-cyan-500 border-cyan-500/20 hover:bg-cyan-500/10",
      };
    case "reddit":
      return {
        icon: <MessageSquare className="h-3.5 w-3.5" />,
        style: "bg-orange-500/10 text-orange-500 border-orange-500/20 hover:bg-orange-500/10",
      };
    default:
      return {
