...
export default function EventsPage() {
  ...
  const [selectedTrigger, setSelectedTrigger] =
    useState<TriggerDefinition | null>(null);
            ...
            <DialogContent className="max-w-[95vw] xs:max-w-[340px] sm:max-w-[450px] md:max-w-[520px] rounded-2xl bg-card border-border shadow-lg p-4 sm:p-5 md:p-6">
              <DialogHeader>
                ...
                <DialogDescription className="text-[12px] sm:text-[13px] text-muted-foreground">
                  Choose a real-time event to watch and configure its
                  parameters.
                </DialogDescription>
              </DialogHeader>
                  ...
                  <>
                    ...
                    {filteredTriggers.length > 0 && searchQuery && (
                      <p className="text-[10px] sm:text-[11px] text-muted-foreground px-1.5 sm:px-2">
                        Found {filteredTriggers.length} trigger
                  ...
                  <div className="space-y-4 sm:space-y-5">
                    <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded-2xl bg-muted/30 border border-border/50">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-medium text-[12px] sm:text-[13px] text-foreground truncate">
                      ...
                      <Button
                        className="text-[11px] sm:text-[12px] h-7 sm:h-8 px-2 font-medium"
                        onClick={() => {
                          setSelectedTrigger(null);
                          setSearchQuery("");
                        }}
                      ...
                      {selectedTrigger.configFields.length === 0 ? (
                        <div className="flex flex-col items-center justify-center gap-2 py-5 bg-muted/10 rounded-2xl border border-border/50">
                          ...
                          <p className="text-[12px] sm:text-[13px] font-medium text-muted-foreground">
                            Ready to subscribe, no configuration needed.
                          </p>
                        </div>
                      ) : (
                        selectedTrigger.configFields.map((field) => (
                          <div className="space-y-1.5" key={field.name}>
                            <Label
                              className="text-[12px] sm:text-[13px] font-medium text-foreground/80"
                              htmlFor={field.name}
                            >
                              {field.label}{" "}
                              {field.required && (
                            ...
                            />
                            <p className="text-[11px] sm:text-[12px] text-muted-foreground pl-1">
                              {field.description}
                            </p>
                          </div>
                        ))
                  ...
                  <div className="grid grid-cols-1 gap-1.5 sm:gap-2 max-h-[280px] xs:max-h-[320px] sm:max-h-[380px] md:max-h-[450px] overflow-y-auto pr-1.5 sm:pr-2 custom-scrollbar">
                    {filteredTriggers.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-6 sm:py-8 gap-2 text-center">
                        ...
                        <p className="text-[11px] sm:text-[12px] md:text-[13px] font-medium text-muted-foreground">
                          No triggers found
                        </p>
                        <p className="text-[10px] sm:text-[11px] md:text-[12px] text-muted-foreground/70">
                      ...
                      filteredTriggers.map((t) => (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <h4 className="font-medium text-[11px] sm:text-[12px] md:text-[13px] text-foreground truncate">
                              {t.name}
                            </h4>
                            <p className="text-[10px] sm:text-[11px] md:text-[12px] text-muted-foreground line-clamp-1">
                              {t.description}
                            </p>
                          </div>
              ...
              <DialogFooter className="gap-2 pt-2 border-t border-border sm:space-x-0">
                <Button
                  className="h-8 sm:h-9 text-[12px] sm:text-[13px] rounded-[10px] font-medium"
                  onClick={() => {
                    setIsCreateDialogOpen(false);
                    setSelectedTrigger(null);
                    setSearchQuery("");
        ...
        <div className="grid grid-cols-1 items-start gap-4 sm:gap-6 lg:grid-cols-12 pb-20">
          ...
          <div className="lg:col-span-4 space-y-3 sm:space-y-4">
            <div className="flex items-center justify-between px-1 sm:px-2">
              ...
              <Badge
                className="rounded-full bg-muted text-muted-foreground border border-border px-1.5 py-0 font-medium text-[10px] sm:text-[11px]"
                variant="secondary"
              >
            ...
            {isTriggersLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-8 sm:py-12 text-center border-2 border-dashed rounded-2xl border-border bg-muted/10 gap-2 sm:gap-3 px-4 sm:px-6 relative overflow-hidden group">
                ...
                <div className="relative z-10">
                  <p className="text-[12px] sm:text-[13px] font-semibold text-foreground/80">
                    No active subscriptions
                  </p>
                  <p className="text-[11px] sm:text-[12px] text-muted-foreground mt-0.5 max-w-[200px]">
                    Set up your first trigger to stream events.
                  </p>
                </div>
              </div>
            ) : (
              <div className="space-y-2 sm:space-y-4">
                {activeTriggers.map((t, idx) => {
                  const rawSlug =
                    t.triggerSlug ||
                    (t as any).triggerName ||
                    (t as any).name ||
                    (t as any).id ||
                    "";
                  const def = availableTriggers.find(
                      ...
                      <Card className="border-border bg-card shadow-sm hover:bg-muted/50 transition-colors group rounded-2xl relative overflow-hidden">
                        <CardHeader className="p-2 sm:p-3 flex flex-row items-center gap-2 sm:gap-3 space-y-0 text-foreground">
                          ...
                          <Button
                            ...
                            onClick={() =>
                              triggerId && handleDeleteTrigger(triggerId)
                            }
                            size="icon"
                            variant="ghost"
                          >
                            {isActionLoading === triggerId ? (
                              <LoaderIcon className="size-3.5 sm:size-4 animate-spin text-destructive" />
                            ) : (
                ...
                {isLoading ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-16 sm:py-20 text-center bg-muted/10 border border-border rounded-2xl gap-3 sm:gap-4 relative group overflow-hidden">
                    ...
                    <div className="relative z-10 space-y-1">
                      <h3 className="text-[14px] sm:text-[16px] font-semibold text-foreground/80">
                        Waiting for signals
                      </h3>
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex flex-col xs:flex-row xs:items-center gap-0.5 sm:gap-2 mb-0.5">
                                  <h4 className="font-medium text-[13px] sm:text-[14px] tracking-tight text-foreground/90 truncate">
                                    {displaySlug}
                                  </h4>
                                  <Badge
                                    className={cn(
                                      "w-fit font-medium rounded-[6px] text-[9px] sm:text-[10px] px-1.5 py-0 uppercase",
                                ...
                                <div className="flex flex-wrap items-center font-normal text-[11px] sm:text-[12px] gap-1.5 sm:gap-2 text-muted-foreground">
                                  <span className="flex items-center gap-1">
                                    <Clock
                                      className="sm:size-3 text-muted-foreground/60"
                                      size={10}
                                    />
                                    {new Date(
                                      event.createdAt
                                ...
                                <div className="p-2 sm:p-3 space-y-2 sm:space-y-3">
                                  <div className="space-y-1.5">
                                    <div className="flex items-center justify-between">
                                      <h5 className="text-[10px] sm:text-[11px] uppercase tracking-widest font-semibold text-muted-foreground">
                                        Payload Inspection
                                      </h5>
                                    </div>
                                    <div className="rounded-[12px] border border-border bg-background p-2 sm:p-3 font-mono text-[10px] sm:text-[12px] text-foreground shadow-inner relative">
                                      <div className="max-h-[300px] sm:max-h-[500px] overflow-y-auto custom-scrollbar overflow-x-hidden">
                                        <pre className="relative z-10 text-foreground whitespace-pre-wrap break-all pr-2">
                                          {JSON.stringify(
                                            event.payload,
                                            null,
                                            2
