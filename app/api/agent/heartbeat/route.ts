...
export async function POST(req: NextRequest) {
  ...
  if (receiver) {
    ...
    if (type === "weekly_synthesis") {
      return NextResponse.json({
        ok: true,
        type: "weekly_synthesis",
        workflowRunId: triggered?.workflowRunId ?? null,
      });
    }
  ...
  if (type === "weekly_synthesis") {
    ...
    if (!triggered) {
      console.error(
        `[Heartbeat] Dev synthesis workflow trigger failed for ${userId}`
      );
    }
    return NextResponse.json({
      ok: true,
  ...
  if (!triggered) {
    console.error(
      `[Heartbeat] Dev workflow trigger failed for ${userId}. Check QSTASH_TOKEN and BASE_URL.`
    );
  }
  return NextResponse.json({
    ok: true,
