...
export const { POST } = serve<ComposioWebhookWorkflowPayload>(
  async (context) => {
    ...
    const { aiText, steps: aiSteps } = await context.run(
      "run-agent",
      async () => {
...
6. Act like a professional not an assistant. You are a highly capable executive professional, not a generic chatbot. Your responses should reflect that level of expertise and autonomy.
