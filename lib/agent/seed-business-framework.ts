...
export async function seedBusinessFramework(userId: string): Promise<boolean> {
  ...
  if (!redis) {
    console.warn("[seedBusinessFramework] Redis not configured, skipping seed.");
    return false;
  }
  ...
  async function createEntity(data: {
    ...
    await redis.set(`kg:${userId}:entity:${id}`, JSON.stringify(entity));
    await redis.sadd(`kg:${userId}:entities`, id);
    return id;
  }
  ...
  async function createRelation(data: {
    ...
    await redis.set(`kg:${userId}:relation:${id}`, JSON.stringify(relation));
    await redis.sadd(`kg:${userId}:relations`, id);
    await redis.sadd(`kg:${userId}:out:${data.fromEntityId}`, id);
    await redis.sadd(`kg:${userId}:in:${data.toEntityId}`, id);
  }
  ...
  try {
    ...
    const bmcHeaderId = await createEntity({
      ...
      summary: "Strategic management template for developing new or documenting existing business models.",
    ...
    const valPropId = await createEntity({
      ...
      summary: "The collection of products and services a business offers to meet the needs of its customers.",
    ...
    const custSegId = await createEntity({
      ...
      entityType: "bmc_block",
      summary: "The different groups of people or organizations an enterprise aims to reach and serve.",
    ...
    await createRelation({ fromEntityId: bmcHeaderId, toEntityId: valPropId, relationType: "contains_component", weight: 1.0 });
    await createRelation({ fromEntityId: bmcHeaderId, toEntityId: custSegId, relationType: "contains_component", weight: 1.0 });
    await createRelation({ fromEntityId: bmcHeaderId, toEntityId: revStreamId, relationType: "contains_component", weight: 1.0 });
    await createRelation({ fromEntityId: bmcHeaderId, toEntityId: costStructId, relationType: "contains_component", weight: 1.0 });

    // 2. KPI TREE ENTITIES & RELATIONS
    const arrId = await createEntity({
      name: "Annual Recurring Revenue (ARR)",
      entityType: "metric",
      summary: "Annualized value of recurring revenue from active SaaS subscriptions.",
      tags: ["kpi", "finance", "growth", "saas"],
      aliases: ["ARR", "Run-rate Revenue"],
      facts: [
        "Top-line financial growth metric.",
    ...
    const cacId = await createEntity({
      ...
      entityType: "metric",
      summary: "Total cost of sales and marketing required to acquire a single paying customer.",
    ...
    const ltvId = await createEntity({
      ...
      entityType: "metric",
      summary: "Total revenue a business expects to earn from a single customer over their lifetime.",
    ...
    const nrrId = await createEntity({
      ...
      entityType: "metric",
      summary: "Percentage of recurring revenue retained from existing customers over a period.",
    ...
    await createRelation({ fromEntityId: cacId, toEntityId: ltvId, relationType: "feeds_into", weight: 0.9, evidence: "CAC efficiency dictates LTV return threshold" });
    await createRelation({ fromEntityId: nrrId, toEntityId: arrId, relationType: "drives_growth", weight: 0.95, evidence: "High NRR compounds ARR compounding without net new acquisition" });
    await createRelation({ fromEntityId: revStreamId, toEntityId: arrId, relationType: "monetizes", weight: 1.0 });

    // 3. OKR & WBR TEMPLATES
    const okrTemplateId = await createEntity({
      name: "Company OKR Template",
      entityType: "template",
      summary: "Quarterly Objectives and Key Results framework for cascading company goals into department targets.",
    ...
    const wbrTemplateId = await createEntity({
      ...
      entityType: "template",
      summary: "Weekly scorecard structure for reviewing revenue performance, pipeline, active incidents, and operational bottlenecks.",
    ...
    await createRelation({ fromEntityId: bmcHeaderId, toEntityId: okrTemplateId, relationType: "guides_planning", weight: 0.8 });
    await createRelation({ fromEntityId: bmcHeaderId, toEntityId: wbrTemplateId, relationType: "tracks_execution", weight: 0.8 });

    // Mark as seeded in Redis
    await redis.set(seededMarkerKey, "true");
    console.log(`[seedBusinessFramework] Successfully seeded Business Model Canvas, KPI Tree, and OKR/WBR templates for user ${userId}.`);
    return true;
  } catch (error) {
    console.error("[seedBusinessFramework] Failed to seed business framework:", error);
    return false;
  }
}
