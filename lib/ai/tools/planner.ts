...
async function readPlan(redis: Redis, userId: string, planId: string): Promise<PlanningItem | null> {
...
export const createPlan = ({ userId }: { userId: string }) =>
  tool({
    ...
    inputSchema: z.object({
      title: z.string().describe("The high-level goal or title of this plan/checklist."),
      description: z.string().optional().default("").describe("A brief summary of what this plan covers."),
      tasks: z.array(z.string()).describe("A list of initial checklist tasks in execution order."),
    }),
...
export const addPlanTask = ({ userId }: { userId: string }) =>
  tool({
    description: "Add one or more checklist tasks to an existing plan or todo list.",
    inputSchema: z.object({
      planId: z.string().describe("The ID of the plan to modify."),
      tasks: z.array(z.string()).describe("List of new task descriptions to add to the checklist."),
    }),
...
export const updatePlanTask = ({ userId }: { userId: string }) =>
  tool({
    description: "Update the status (complete/pending) or notes of a specific task within a plan/checklist.",
    inputSchema: z.object({
      planId: z.string().describe("The ID of the plan."),
      taskId: z.string().describe("The ID of the specific task/checklist item to update."),
      status: z.enum(["pending", "completed"]).optional().describe("Mark the task as completed or pending."),
      notes: z.string().optional().describe("Optional progress details or execution logs for this specific task."),
    }),
    execute: async ({ planId, taskId, status, notes }) => {
      ...
      if (!taskUpdated) {
        return { success: false, error: `Task item "${taskId}" not found in this plan.` };
      }

      // Automatically determine if the whole plan is completed
      const allCompleted = updatedTasks.every((t) => t.status === "completed");
      const computedStatus = allCompleted ? "completed" as const : current.status;
...
export const listPlans = ({ userId }: { userId: string }) =>
  tool({
    description: "List all active, completed, or archived plans, todo lists, and checklists.",
    inputSchema: z.object({
      status: z.enum(["active", "completed", "archived"]).optional().describe("Filter plans by current status."),
      limit: z.number().optional().default(20).describe("Maximum number of plans to return."),
    }),
    execute: async ({ status, limit }) => {
      ...
      return { success: true, plans: enhancedPlans, count: enhancedPlans.length };
    },
  });
