  ...
  tool({
    ...
    inputSchema: z.object({
      ...
      query: z
        .string()
        .optional()
        .describe("SQL query to execute (required for action='query')"),
      params: z.array(z.any()).optional().describe("Query parameters"),
      tableName: z
        .string()
        .optional()
        .describe("Schema-qualified table name (e.g. public.users)"),
      schema: z
        .string()
        .optional()
        .default("public")
      ...
      explain: z
        ...
        .optional()
        .default(false)
        .describe("Prefix with EXPLAIN ANALYZE for query action"),
      limit: z
        .number()
        .int()
        .optional()
      ...
      pid: z
        ...
        .int()
        .optional()
        .describe("PID to kill for kill_connections"),
      timeoutMs: z
        .number()
        .int()
        .optional()
        .default(30_000)
        .describe("Query timeout in ms"),
    }),
    execute: async ({
      action,
      connectionString,
      ...
      const client = new Client({
        connectionString,
        statement_timeout: timeoutMs || 30_000,
      });
      try {
        ...
        switch (action) {
          ...
          case "describe_table": {
            if (!tableName) {
              return {
                success: false,
                error: "tableName required (e.g. public.users)",
              };
            }
            const [schemaName, tbl] = tableName.includes(".")
              ? tableName.split(".")
          ...
          case "analyze_query": {
            ...
            return {
              success: true,
              plan: res.rows[0]?.["QUERY PLAN"] || res.rows,
            };
          }
          ...
          case "kill_connections": {
            ...
            if (!database) {
              return { success: false, error: "database or pid required" };
            }
            const res = await client.query(
            ...
            return {
              success: true,
              database,
              terminated: (res.rows || []).filter(
                (r: any) => r.pg_terminate_backend
              ).length,
            };
          }
...
export const mysqlQuery = ({ userId }: { userId: string }) =>
  tool({
    ...
    inputSchema: z.object({
      ...
      uri: z
        .string()
        .describe("MySQL connection URI (mysql://user:pass@host:3306/db)"),
      ...
      explain: z
        ...
        .optional()
        .default(false)
        .describe("Prefix with EXPLAIN for query action"),
      limit: z
        .number()
        .int()
        .optional()
      ...
      variablePattern: z
        .string()
        .optional()
        .describe("Variable name pattern filter"),
    }),
    execute: async ({
      action,
      uri,
      ...
      try {
        ...
        switch (action) {
          ...
          case "kill_query": {
            if (!processId) {
              return { success: false, error: "processId required" };
            }
          ...
          case "engine_status": {
            ...
            const [qps] = await connection.execute(
  ...
  tool({
    ...
    inputSchema: z.object({
      ...
      document: z
        .record(z.any())
        .optional()
        .describe("Single document for insert/update"),
      documents: z
        .array(z.record(z.any()))
        .optional()
        .describe("Documents for insert_many/bulk_write"),
      ...
      indexSpec: z
        .record(z.any())
        .optional()
        .describe("Index specification (e.g. { field: 1 })"),
      ...
      command: z
        .record(z.any())
        .optional()
        .describe("Database command for run_command"),
      upsert: z
        .boolean()
        .optional()
        .default(false)
        .describe("Upsert for update operations"),
      ordered: z
        .boolean()
        .optional()
        .default(true)
        .describe("Ordered bulk write"),
    }),
    execute: async ({
      mongoAction,
      uri,
        ...
        switch (mongoAction) {
          ...
          case "insert_one": {
            if (!coll || !document) {
              return {
                success: false,
                error: "Collection and document required",
              };
            }
          ...
          case "insert_many": {
            if (!coll || !documents?.length) {
              return {
                success: false,
                error: "Collection and documents required",
              };
            }
          ...
          case "update_one": {
            if (!coll || !filter || !update) {
              return {
                success: false,
                error: "Collection, filter, and update required",
              };
            }
          ...
          case "delete_one": {
            if (!coll || !filter) {
              return {
                success: false,
                error: "Collection and filter required",
              };
            }
          ...
          case "aggregate": {
            if (!coll || !pipeline?.length) {
              return {
                success: false,
                error: "Collection and pipeline required",
              };
            }
          ...
          case "distinct": {
            if (!coll || !fieldName) {
              return {
                success: false,
                error: "Collection and fieldName required",
              };
            }
          ...
          case "list_databases": {
            ...
            return {
              success: true,
              databases: dbs.databases.map((d: any) => ({
                name: d.name,
          ...
          case "create_index": {
            if (!coll || !indexSpec) {
              return {
                success: false,
                error: "Collection and indexSpec required",
              };
            }
          ...
          case "drop_index": {
            if (!coll || !indexName) {
              return {
                success: false,
                error: "Collection and indexName required",
              };
            }
          ...
          case "bulk_write": {
            ...
            return {
              success: false,
              error:
                "Use insert_many, update_many, or delete_many for bulk operations",
            };
          }
          ...
          case "watch_collection": {
            ...
            return {
              success: false,
              error:
                "watch_collection is a streaming operation. Use find with a sort filter instead.",
            };
          }
