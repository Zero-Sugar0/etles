...
export const analyzeContract = ({ userId }: { userId: string }) =>
  tool({
    ...
    inputSchema: z.object({
      ...
      context: z.string().optional().describe("Optional context about the user's role (e.g. 'I am the licensor', 'I am the employee')"),
    }),
    execute: async ({ contractText, context }) => {
      try {
        ...
        const { object } = await generateObject({
          ...
          schema: z.object({
            metadata: z.object({
              documentType: z.string().describe("Type of legal document detected"),
            ...
            negotiationScore: z.object({
              overall: z.number().min(1).max(10).describe("1=highly unfavorable, 10=highly favorable"),
              balance: z.string(),
              keyNegotiationPoints: z.array(z.string()),
            }),
            summary: z.string(),
            actionItems: z.array(z.string()).describe("Concrete recommended actions for the user"),
          }),
...
export const compareContracts = ({ userId }: { userId: string }) =>
  tool({
    ...
    inputSchema: z.object({
      ...
      context: z.string().optional().describe("Optional context about the user's position"),
    }),
        ...
        const { object } = await generateObject({
          ...
          schema: z.object({
            sectionDiffs: z.array(z.object({
              section: z.string(),
              type: z.enum(["addition", "deletion", "modification", "unchanged"]),
            ...
            negotiationStrategy: z.object({
              recommendedStance: z.enum(["accept", "counter", "reject", "needs_discussion"]),
...
export const extractClauses = ({ userId }: { userId: string }) =>
  tool({
    ...
    inputSchema: z.object({
      ...
      ).describe("Which clause types to extract. Use 'all' to extract every clause type."),
    }),
      ...
      try {
        ...
        const { object } = await generateObject({
          ...
          schema: z.object({
            ...
            missingClauses: z.array(z.string()).describe("Standard clause types that appear to be missing from this contract"),
          }),
...
export const complianceCheck = ({ userId }: { userId: string }) =>
  tool({
    ...
    inputSchema: z.object({
      contractText: z.string().describe("The contract or legal document to check"),
      frameworks: z.array(
        z.enum(["gdpr", "ccpa", "hipaa", "soc2", "pci_dss", "general_data_protection"]),
      ).describe("Compliance frameworks to check against"),
      businessType: z.string().optional().describe("Type of business (e.g. SaaS provider, healthcare, fintech)"),
    }),
        ...
        const { object } = await generateObject({
          ...
          schema: z.object({
            ...
            frameworkResults: z.array(z.object({
              ...
              nonCompliantClauses: z.array(z.object({
                clause: z.string(),
                issue: z.string(),
...
export const redlineContract = ({ userId }: { userId: string }) =>
  tool({
    ...
    inputSchema: z.object({
      ...
      partyPerspective: z.enum(["buyer", "seller", "employee", "employer", "licensor", "licensee", "service_provider", "client", "neutral"]).optional().default("neutral").describe("Your perspective in the contract"),
      aggressiveness: z.enum(["conservative", "moderate", "aggressive"]).optional().default("moderate").describe("How aggressive to be in proposed changes"),
      specificConcerns: z.string().optional().describe("Any specific concerns to focus on"),
    }),
    execute: async ({ contractText, partyPerspective, aggressiveness, specificConcerns }) => {
      try {
        const model = getLanguageModel("anthropic/claude-sonnet-4.5");
        const { object } = await generateObject({
          model,
          schema: z.object({
            ...
            newClausesToAdd: z.array(z.object({
              title: z.string(),
              proposedText: z.string(),
              reason: z.string(),
              priority: z.enum(["high", "medium", "low"]),
            })),
            clausesToRemove: z.array(z.object({
              clause: z.string(),
              reason: z.string(),
...
export const obligationTracker = ({ userId }: { userId: string }) =>
  tool({
    ...
    inputSchema: z.object({
      ...
      effectiveDate: z.string().optional().describe("The effective date of the contract (ISO format) for timeline calculations"),
    }),
        ...
        const { object } = await generateObject({
          ...
          schema: z.object({
            obligations: z.array(z.object({
              party: z.string(),
              obligation: z.string(),
              category: z.enum(["payment", "delivery", "performance", "reporting", "compliance", "notice", "other"]),
              deadline: z.string().optional(),
              conditionPrecedent: z.string().optional(),
              recurring: z.boolean(),
              frequency: z.string().optional(),
              status: z.enum(["pending", "ongoing", "completed", "triggered"]),
              priority: z.enum(["high", "medium", "low"]),
            ...
            upcomingReminders: z.array(z.string()).describe("Reminders for upcoming obligations based on the effective date"),
...
export const jurisdictionAnalysis = ({ userId }: { userId: string }) =>
  tool({
    ...
    inputSchema: z.object({
      contractText: z.string().describe("The contract text to analyze"),
      userLocation: z.string().optional().describe("The user's country/state for conflict analysis"),
      counterpartyLocation: z.string().optional().describe("The counterparty's country/state"),
    }),
...
export const riskScoring = ({ userId }: { userId: string }) =>
  tool({
    ...
    inputSchema: z.object({
      ...
      userRole: z.string().optional().describe("The user's role/position relative to the contract"),
    }),
    execute: async ({ contractText, userRole }) => {
      try {
        ...
        const { object } = await generateObject({
          model,
          schema: z.object({
            overallScore: z.number().min(0).max(100).describe("0=extremely risky, 100=completely safe"),
            overallVerdict: z.string(),
            categoryScores: z.array(z.object({
              category: z.string(),
              score: z.number().min(0).max(100),
              weight: z.number().min(1).max(5).describe("Importance weight 1-5"),
            ...
            criticalIssues: z.array(z.object({
              ...
              impact: z.string(),
              urgency: z.enum(["immediate", "before_signing", "before_accepting"]),
              recommendation: z.string(),
            })),
