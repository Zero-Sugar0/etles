    ...
    execute: async ({ sandboxId }) => {
      try {
        ...
        if (install.exitCode !== 0) {
          return {
            success: false,
            error: `Playwright install failed: ${install.result}`,
          };
        }
...
export const browserNavigate = ({ userId }: { userId: string }) =>
  tool({
    ...
    inputSchema: z.object({
      ...
      url: z
        .string()
        .describe("Full URL to navigate to. E.g. 'https://example.com'"),
      waitForSelector: z
        .string()
        .optional()
        .describe(
          "CSS selector to wait for before returning. E.g. '#main-content'"
        ),
      sessionId: z
        .string()
        .optional()
        ...
        .describe(
          "Browser session name for persistent state (cookies, login)."
        ),
    }),
  ...
  tool({
    ...
    inputSchema: z.object({
      ...
      url: z
        ...
        .optional()
        .describe(
          "Navigate to this URL first (if not already on the right page)."
        ),
        ...
        .array(
          z.object({
            ...
            selector: z
              ...
              .optional()
              .describe(
                "CSS selector. E.g. '#submit-btn', 'button:has-text(\"Login\")', 'input[name=email]'"
              ),
            text: z
              .string()
              .optional()
              .describe(
                "Text to type/fill, or option label/value for select, or key name for press."
              ),
            scrollX: z
              .number()
              .optional()
            ...
            scrollY: z
              ...
              .optional()
              .describe(
                "Vertical scroll amount in pixels (for scroll action). Use negative to scroll up."
              ),
          })
        )
      ...
      try {
        ...
        const steps = actions
          .map((a, i) => {
            ...
            switch (a.type) {
              case "click":
                return sel ? `await ${sel}.click();` : "";
              case "dblclick":
                return sel ? `await ${sel}.dblclick();` : "";
              case "rightclick":
                return sel ? `await ${sel}.click({ button: 'right' });` : "";
              case "hover":
                return sel ? `await ${sel}.hover();` : "";
              case "focus":
                return sel ? `await ${sel}.focus();` : "";
              case "clear":
                return sel ? `await ${sel}.clear();` : "";
              case "check":
                return sel ? `await ${sel}.check();` : "";
              case "uncheck":
                return sel ? `await ${sel}.uncheck();` : "";
              case "submit":
                return sel
                  ? `await ${sel}.evaluate(el => el.closest('form')?.submit());`
                  : "";
              case "type":
                return `await page.keyboard.type(${JSON.stringify(a.text ?? "")});`;
              case "fill":
                return sel
                  ? `await ${sel}.fill(${JSON.stringify(a.text ?? "")});`
                  : "";
              case "press":
                return `await page.keyboard.press(${JSON.stringify(a.text ?? "Enter")});`;
              case "select":
                return sel
                  ? `await ${sel}.selectOption(${JSON.stringify(a.text ?? "")});`
                  : "";
              case "scroll":
                return a.selector
              ...
              case "wait":
                return sel ? `await ${sel}.waitFor({ timeout: 8000 });` : "";
              default:
                return "";
            }
          })
    ...
    inputSchema: z.object({
      ...
      extract: z.object({
        ...
        links: z
          .boolean()
          .optional()
          .describe("Extract all hyperlinks with text and href."),
        tables: z
          .boolean()
          .optional()
          .describe("Extract all tables as arrays of row objects."),
        metadata: z
          .boolean()
          .optional()
          .describe("Extract page metadata (title, description, og tags)."),
        forms: z
          .boolean()
          .optional()
          .describe("Extract all form fields and their attributes."),
        images: z
          .boolean()
          .optional()
          .describe("Extract all image src + alt text."),
        customSelector: z
          .string()
          .optional()
          .describe(
...
(async () => {
  ...
  ${
    extract.customSelector
      ? `result.custom = await page.evaluate((sel, attr) =>
  ...
  tool({
    ...
    inputSchema: z.object({
      ...
      urls: z
        ...
        .min(1)
        .max(10)
        .describe("URLs to open in parallel. Max 10."),
      extractSelector: z
        .string()
        .optional()
        .describe(
  ...
  tool({
    ...
    inputSchema: z.object({
      ...
      filePath: z
        .string()
        .describe(
          "Absolute path of the file inside the sandbox. E.g. '/home/daytona/resume.pdf'"
        ),
    }),
    execute: async ({
      sandboxId,
  ...
  tool({
    ...
    inputSchema: z.object({
      ...
      region: z
        ...
        .optional()
        .describe("Capture only a region instead of the full screen."),
    ...
    execute: async ({ sandboxId, region, compressed }) => {
      try {
        ...
        return {
          ...
          format: "png",
          message:
            "Screenshot taken. Pass the `image` field to a vision model to interpret coordinates.",
        };
      } catch (error: any) {
        return { success: false, error: error?.message ?? String(error) };
      }
          ...
          z.discriminatedUnion("type", [
            ...
            z.object({
              ...
              key: z
                .string()
                .describe(
                  "Key name e.g. 'Return', 'Escape', 'Tab', 'BackSpace'"
                ),
            ...
            z.object({
              ...
              keys: z
                .string()
                .describe("Hotkey combo e.g. 'ctrl+c', 'ctrl+v', 'alt+tab'"),
            }),
          ])
        )
        ...
        for (const action of actions) {
          switch (action.type) {
            ...
            case "drag":
              ...
              results.push(
                `dragged (${action.startX},${action.startY}) → (${action.endX},${action.endY})`
              );
              break;
            case "scroll":
              ...
              results.push(
                `scrolled ${action.direction} at (${action.x},${action.y})`
              );
              break;
            case "type":
              ...
              break;
            case "press":
              await sandbox.computerUse.keyboard.press(
                action.key,
                action.modifiers
              );
              results.push(
                `pressed ${action.key}${action.modifiers?.length ? "+" + action.modifiers.join("+") : ""}`
              );
              break;
            case "hotkey":
