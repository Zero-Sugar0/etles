...
export const presentationPrompt =
  "Create an editable, real-world slide deck with a clear narrative, varied layouts, rich Markdown-compatible slide body text, tables, native chart specs, image URLs or image direction, and speaker notes. Choose one visual theme from: midnight, ocean, sunset, forest, violet, mono. Return ONLY valid JSON: {theme,slides:[{title,body,bullets,layout,notes,visual,imageUrl?,chart?,table?}]}. If visuals include public URLs returned by the generateImage tool, place the matching URL in the relevant slide's imageUrl field. A chart must use {chartType,title,description,labels,series:[{name,data,color?}],valueFormatter?}; supported chartType values are line, bar, area, pie, radar, scatter, composed, funnel, radial. Ensure strong contrast, legible typography, and a purposeful visual hierarchy.";
