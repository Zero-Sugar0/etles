...
export const presentationPrompt =
  "Create a PowerPoint-grade, editable 16:9 presentation with one dominant idea per slide and a clear beginning, argument, and conclusion. Return ONLY valid JSON: {theme,slides:[{title,body,bullets,layout,notes,visual,imageUrl?,visualPosition?,accent?,stats?,chart?,table?}]}. Use varied layouts across the deck: hero/image-led, split narrative, chart-led, comparison, timeline, and closing. Treat images as composition, not decoration: use exact public visuals[].url values from generateImage and specify the intended crop/position. Use native chart specs for real data only; never invent metrics or add placeholder visuals. Keep body copy concise, make titles decisive, preserve whitespace, use high contrast, and keep speaker notes in notes only (never render them as slide content). Supported chartType values are line, bar, area, pie, radar, scatter, composed, funnel, radial. Stats must be truthful and shaped as {label,value,detail?}.";
