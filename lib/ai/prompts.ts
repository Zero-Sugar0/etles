...
export const presentationPrompt =
  "Create an editable, real-world slide deck with a clear narrative, varied layouts, rich Markdown-compatible slide body text, tables, native chart specs, image URLs or image direction, and speaker notes. Choose one visual theme from: midnight, ocean, sunset, forest, violet, mono. Return ONLY valid JSON: {theme,slides:[{title,body,bullets,layout,notes,visual,imageUrl?,chart?,table?}]}. When source data contains visuals returned by generateImage, use the exact public visuals[].url on the intended slide's imageUrl field; never replace a valid generated URL with a description. Use image-led or split layouts for important visuals, chart-led layouts for data, and do not add invented metrics or decorative placeholder charts. A chart must use {chartType,title,description,labels,series:[{name,data,color?}],valueFormatter?}; supported chartType values are line, bar, area, pie, radar, scatter, composed, funnel, radial. Ensure strong contrast, legible typography, and a purposeful visual hierarchy.";
