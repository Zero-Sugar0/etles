...
if (!headersOutput.includes('HEADER 1') || !headersOutput.includes('Header 2') || !headersOutput.includes('Header 3')) {
  console.error('FAIL: Headers not parsed correctly.');
  process.exit(1);
}

console.log('\n--- TESTING INLINE FORMATTING ---');
const inlineInput = `This has **bold**, *italic*, ~~strikethrough~~ and \`inline code\`.`;
const inlineOutput = renderMarkdown(inlineInput);
console.log(inlineOutput);
if (!inlineOutput.includes('\x1b[1mbold\x1b[22m') || !inlineOutput.includes('\x1b[3mitalic\x1b[23m')) {
...
if (!listOutput.includes('\x1b[32m•\x1b[0m Bullet item 1') || !listOutput.includes('\x1b[34m│\x1b[0m \x1b[3mBlockquote line\x1b[0m')) {
...
if (!tableOutput.includes('┌') || !tableOutput.includes('Header A') || !tableOutput.includes('Value B2')) {
