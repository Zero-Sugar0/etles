...
function highlightCodeBasic(code: string): string {
  ...
  return code
    .replace(
      /(const|let|var|function|return|import|export|from|class|extends|if|else|for|while|await|async|typeof|try|catch|new|throw)\b/g,
      "\x1b[35m$1\x1b[0m"
    ) // keywords (magenta)
...
export function highlightCode(code: string, lang: string): string {
  if (highlighter) {
    try {
      ...
      const tokenLines = Array.isArray(tokenResult)
        ? tokenResult
        : (tokenResult.tokens ?? []);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.trim().startsWith("```")) {
      if (inCodeBlock) {
        ...
        renderedLines.push(
          "\x1b[90m┌" + "─".repeat(Math.min(terminalWidth - 2, 78)) + "┐\x1b[0m"
        );
        ...
        for (const codeLine of codeLines) {
          ...
          const padding = Math.max(
            0,
            Math.min(terminalWidth - 2, 78) - visible.length
          );
          renderedLines.push(
            "\x1b[90m│\x1b[0m " +
              codeLine +
    ...
    if (line.startsWith("#")) {
      ...
      if (match) {
        ...
        if (level === 1) {
          ...
          renderedLines.push(
            `\x1b[36m${"━".repeat(Math.min(terminalWidth, formattedText.length))}\x1b[0m`
          );
        } else if (level === 2) {
          renderedLines.push(`\n\x1b[1;33m${formattedText}\x1b[0m`);
          renderedLines.push(
            `\x1b[90m${"─".repeat(Math.min(terminalWidth, formattedText.length))}\x1b[0m`
          );
        } else {
    ...
    if (line.trim().startsWith(">")) {
      ...
      renderedLines.push(
        `\x1b[34m│\x1b[0m \x1b[3m${inlineFormatting(quoteText)}\x1b[0m`
      );
      continue;
    }
    ...
    if (line.trim().match(/^([*+-]|\d+\.)\s+/)) {
      ...
      if (match) {
        ...
        renderedLines.push(
          " ".repeat(indent) +
            `\x1b[32m${bulletSymbol}\x1b[0m ${inlineFormatting(content)}`
        );
        continue;
      }
    }
...
function renderTable(tableLines: string[], terminalWidth: number): string {
  ...
  const drawLine = (left: string, mid: string, right: string, dash: string) => {
    return (
      "\x1b[90m" +
      left +
      colWidths.map((w) => dash.repeat(w + 2)).join(mid) +
      right +
      "\x1b[0m\n"
    );
  };
  ...
  for (let r = 0; r < rows.length; r++) {
    ...
    tableStr +=
      "\x1b[90m│\x1b[0m" +
      formattedCells.join("\x1b[90m│\x1b[0m") +
      "\x1b[90m│\x1b[0m\n";
  }
