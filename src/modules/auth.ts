...
export async function saveCredentials(creds: CredentialData): Promise<void> {
  ...
  if (keytar) {
    try {
      if (creds.apiKey) {
        await keytar.setPassword("etles-cli-agent", "apiKey", creds.apiKey);
      }
      if (creds.token) {
        await keytar.setPassword("etles-cli-agent", "token", creds.token);
      }
...
export async function validateCredentials(
  ...
  password?: string,
  apiKey?: string
): Promise<{ success: boolean; error?: string; data?: CredentialData }> {
  if (apiKey) {
    if (!apiKey.startsWith("api-key-") && apiKey.length < 20) {
      return {
        success: false,
        error: "Invalid API key format. Must be at least 20 chars.",
      };
    }
    // Success simulation
  ...
  return {
    success: false,
    error: "Either API key or Email + Password is required.",
  };
}

export async function silentTokenRefresh(
  currentCreds: CredentialData
): Promise<CredentialData> {
  // If the session is within 5 minutes of expiring or already expired, silently refresh it
  if (
    currentCreds.expiresAt &&
    Date.now() > currentCreds.expiresAt - 1000 * 60 * 5
  ) {
