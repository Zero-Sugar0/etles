import { validateCredentials, saveCredentials, getCredentials, deleteCredentials } from './modules/auth';
import { getConfig, setConfigKey, resetConfig } from './modules/config';
import { setMemory, getMemory, listMemory, deleteMemory, clearMemory } from './modules/memory';
...
async function runTests() {
  console.log('\x1b[1;36m=== STARTING ETLES CLI INTEGRATION TEST SUITE ===\x1b[0m\n');
  ...
  try {
    ...
    assert(credsEmpty === null, 'Credentials successfully cleared and verified empty');

    if (validateRes.data) {
      await saveCredentials(validateRes.data);
      const credsSaved = await getCredentials();
      assert(credsSaved?.user?.email === 'test@etles.ai', 'Credentials successfully stored and retrieved');
    }
  } catch (e: unknown) {
  ...
  try {
    ...
    config = getConfig();
    assert(config.defaultAgent === 'sdr', 'Config setter successfully writes defaultAgent to SDR');
    resetConfig();
  } catch (e: unknown) {
  ...
  try {
    ...
    assert(val?.value === 'Jules' && val.type === 'string', 'Memory key successfully stored and verified with get');

    const list = listMemory();
    assert(list.length === 1 && list[0].key === 'user_name', 'Memory list correctly returns stored key');

    const deleted = deleteMemory('user_name');
    assert(deleted && listMemory().length === 0, 'Memory key deletion functions correctly');
  } catch (e: unknown) {
  ...
  try {
    ...
    assert(tool?.icon === '🐚' && !!tool.schema, 'Shell execution tool inspected successfully with valid schema');
  } catch (e: unknown) {
...
`;
    ...
    assert(rendered.includes('SYSTEM REPORT'), 'Headers capitalized and styled');
    assert(rendered.includes('┌') && rendered.includes('Database'), 'Table column alignments styled with unicode characters');
  } catch (e: unknown) {
